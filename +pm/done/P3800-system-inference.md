...
- [x] System shows yellow after successful pull
- [x] System shows green after successful switch
- [x] System shows red after failed switch
- [x] System shows yellow when Lock is outdated (dashboard inference)
- [x] System shows gray/unknown only on first connection (before any commands)

### Performance

- [x] No expensive checks on heartbeat (<1ms per host)
- [x] Status updates immediately after command completion
- [x] No `nix build --dry-run` calls during normal operation

### UI/UX

- [x] System tooltip shows inferred status message
- [x] When red: tooltip shows "Switch failed: <error message>"
- [ ] Manual refresh option available (runs expensive check on-demand) - deferred to future
...
- [x] Pull → System yellow (agent sets via SetSystemOutdated)
- [x] Switch after pull → System green (agent sets via SetSystemOk)
- [x] Switch fails → System red (agent sets via SetSystemError)
- [x] Lock outdated → System yellow (dashboard inference in handleHeartbeat)
