...
- [x] Agent computes SHA256 hash of `flake.lock` content
- [x] Agent sends `lock_hash` in heartbeat payload
- [x] Hash computation is cheap (<1ms)
- [x] Works on both NixOS and macOS

### Dashboard Side

- [x] Dashboard fetches latest `flake.lock` from nixcfg repo (GitHub Pages via version.json)
- [x] Cache the latest hash (TTL based on VersionFetcher)
- [x] Compare `host.lock_hash` vs `latest_lock_hash`
- [x] Lock compartment: green if match, yellow if different

### UI/UX

- [x] Lock tooltip shows hash comparison: "host: xxx, latest: yyy"
- [x] Lock never shows green when updates available
- [ ] Lock tooltip shows "X commits behind" (deferred: requires additional GitHub API call)
- [ ] When PR pending: tooltip shows "Merge PR #42 to update" (deferred: P4700)
...
- [x] Agent sends lock_hash in heartbeat
- [x] Dashboard receives and stores lock_hash
- [x] Dashboard fetches latest from GitHub Pages (lockHash in version.json)
- [x] Lock compartment updates correctly
